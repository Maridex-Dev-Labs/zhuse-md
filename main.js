...
const tvCommand = require('./commands/tv');
const broadcastCommand = require('./commands/broadcast');
const gameModeV2Command = require('./commands/game_mode_v2');
const { killbotCommand, risebotsCommand } = require('./commands/killbot');
...
global.channelLink = "https://chat.whatsapp.com/DH2CDvEq03S5KIhhIlZYO9?mode=gi_t";
global.ytch = "ZHUSE MD";
    ...
    try {
        ...
        if (message.message?.buttonsResponseMessage) {
            ...
            if (buttonId === 'channel') {
                await sock.sendMessage(chatId, {
                    text: '📢 *Join our Channel:*\nhttps://chat.whatsapp.com/DH2CDvEq03S5KIhhIlZYO9?mode=gi_t'
                }, { quoted: message });
                return;
        ...
        switch (true) {
            ...
            case userMessage === '.help' || userMessage === '.menu' || userMessage === '.bot' || userMessage === '.list':
                ...
                break;
            case userMessage === '.tv':
                await tvCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            ...
            case userMessage.startsWith('.pair'):
                ...
                commandExecuted = true;
                break;
            case userMessage.startsWith('.game_mode_v2'):
                await gameModeV2Command(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage === '.killbot':
                await killbotCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage === '.risebots':
                await risebotsCommand(sock, chatId, message);
                commandExecuted = true;
                break;
            case userMessage.startsWith('.setstyle'):
                {
                    const parts = userMessage.split(' ');
                    const style = parts[1] ? parts[1].toLowerCase() : '';
                    if (!style) {
                        await sock.sendMessage(chatId, { text: 'Usage: .setstyle <modern|elegant|minimal|gaming|neon|premium>' }, { quoted: message });
                        break;
                    }
                    await helpCommand.setStyleCommand(sock, chatId, style);
                    commandExecuted = true;
                }
                break;
            ...
            case userMessage.startsWith('.sora'):
                ...
                break;
            case userMessage.startsWith('.statussave'):
                {
                    const args = userMessage.split(' ').slice(1);
                    await statusSaveCommand(sock, chatId, message, args);
                }
                commandExecuted = true;
                break;
            default:
